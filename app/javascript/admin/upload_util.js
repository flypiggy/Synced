function uploadImage(container_id, config){
  ...
  const $image_area = $(container_id),
        $upload_element = $(config.upload_element);
  if (!checkLimitValid()) { $(config.template).addClass('disappear') }
  var template;

  $upload_element.on('focusin', function(){
    ...
    readInputFile($input[0], function(file_url) {
      /*选取图片后，预览*/
  ...
  $upload_element.on('click', function(e){
    const $button = $(e.target),
          $this = $(this),
          $destroy_field = $this.find('input[type=checkbox]');
