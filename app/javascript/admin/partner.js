      ...
      ready() {
        uploadImage('#image_area', {
          ...
          changeTemplateAfterSelect: template => {
            const count = $('#image_area').find('.image_preview_area').size() - 1;
            template.find('input[type=file]').attr('name', `partner[logos_attributes][${count}][file]`);
            template.find('input.js_delete_image').attr('name', `partner[logos_attributes][${count}][_destroy]`);
            template.find('input.js_set_to_default').attr('name', `partner[logos_attributes][${count}][default]`);
